...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if a_node == b_node :
            return -1
        elif a_node.is_child(b_node) or b_node.is_child(b_node) :
            return -1
        else :
            print "b"
            a_trace = []
            ...
            while temp != None :
                b_trace.append(temp)
                temp = b_node.get_parent()
                count_b += 1
        i = len(a_trace)-1
        j = len(b_trace)-1
        count = 0
        print "a"
        while a_trace[i] == b_trace[j] :
            i = i - 1
            j = j - 1
            count += 1
        if count_a - count < count_b - count :
            return count_a - count - 1, count_b - count_a
        else :
            return count_b - count - 1, count_a - count_b
